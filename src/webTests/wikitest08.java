...
public class wikitest08 {
    public static void main(String[] args) throws IOException, InterruptedException {
        String url = "https://en.wikipedia.org/wiki/Solihull_plant";
    ...
    public static void pruebaTituloDeLaPagina(String url) throws IOException, InterruptedException {
        String descripcion = "Verificar que el título de la página sea Solihull plant - Wikipedia";
        String titulo = obtenerTituloDeLaPagina(url);
        System.out.println(descripcion);
        System.out.print("Resultado: ");
        if (titulo != null && !titulo.isEmpty() && titulo.equals("Solihull plant - Wikipedia")) {
            printColor("OK\n", "verde");
    ...
    public static void pruebaNumeroDeEnlaces(String url) throws IOException, InterruptedException {
        String descripcion = "Verificar que la página tenga al menos 50 enlaces";
        int numeroDeEnlaces = obtenerNumeroDeEnlaces(url);
        System.out.println(descripcion);
        System.out.print("Resultado: ");
        if (numeroDeEnlaces > 50) {
            printColor("OK\n", "verde");
    ...
    public static void pruebaTextoEspecifico(String url) {
        String descripcion = "Verificar que la página contenga el texto 'Jaguar Land Rover'";
        String html = obtenerHtml(url);
        System.out.println(descripcion);
        System.out.print("Resultado: ");
        if (html.contains("Jaguar Land Rover")) {
            printColor("OK\n", "verde");
        } else {
            printColor("ERROR\n", "rojo");
        }
    ...
    public static void pruebaElementoPorId(String url) {
        String descripcion = "Verificar que la página contenga un elemento con ID 'firstHeading'";
        String html = obtenerHtml(url);
        System.out.println(descripcion);
        System.out.print("Resultado: ");
        if (html.contains("id=\"firstHeading\"")) {
            printColor("OK\n", "verde");
        } else {
            printColor("ERROR\n", "rojo");
        }
